...
- (GREYAppStateTrackerObject *)trackState:(GREYAppState)state
                                forObject:(id)object {
...
- (GREYAppStateTrackerObject *)grey_changeState:(GREYAppState)state
  ...
  GREYFatalAssertWithMessage(operation != kGREYClearState
                             || state == kGREYIdle,
  ...
  GREYFatalAssertWithMessage( (object && !internalObject && !externalObject)
    ...
    // why the logic all written here , left internal and external plain
  return [self grey_performBlockInCriticalSection:^id {
    ...
    // released keys.
    // In some cases, it could lead to a livelock (calling drainUntilIdle inside drainUntilIdle
    // where the first drainUntilIdle sets up an autorelease pool
    // and the second drainUntilIdle never returns
    // because it is expecting the first drainUntilIdle's autorelease pool to release the object
    // so state tracker can return to idle state)
    @autoreleasepool {
      ...
      // only the object is holding onto it strongly.
        ...
        // get  Deallocation tracker and tracker object
      GREYObjectDeallocationTracker *appStateTrackerObjectInternal = nil;
      appStateTrackerObjectInternal = internalObject;
 ...
 [GREYObjectDeallocationTracker deallocationTrackerRegisteredWithObject:object];
      ...
      // There is a possibility that the GREYAppStateTrackerObject isn't part of the map, and object doesn't exist.
      // This is possible when object being removed has already been untracked.
    ...
    //未跟踪对象
      ...
      if (newState == kGREYIdle) {
        ...
        if (appStateTrackerObjectInternal) {//when operation == kGREYTrackState
          objc_setAssociatedObject(appStateTrackerObjectInternal,
                                   @selector(currentState),
                                   appStateTrackerObjectExternal,
...
- (void)grey_clearState {
  [self grey_performBlockInCriticalSection:^id {
    ...
    for (GREYAppStateTrackerObject *externalObject in _externalTrackerObjects) {
      GREYObjectDeallocationTracker *
          internalObject = externalObject.object;
