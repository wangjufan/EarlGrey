...
//public frame superview   boudn self coordinate system
...
+ (NSArray *)touchPathForGestureInView:(UIView *)view
  ...
  GREYThrowOnFailedConditionWithMessage(isnan(startPointPercents.y)
                        ...
                        @"startPointPercents must be NAN or in the range (0, 1) "
                                        @"exclusive");
  GREYThrowOnFailedConditionWithMessage(length > 0, @"Scroll length must be positive and greater than zero.");
...
//kGREYDirectionRight=2
//kGREYDirectionUp=3
//kGREYDirectionDown=4
/**
...
////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////
/**  Content edges for scrolling. 已知目标边+边框 --  边上中点
 ...
 *  by the given @c insets. Note that if width/height is less than the required insets they are set to zero.
...
//innner
/**
 *  Touch path between the given points with the option to cancel the inertia.[ɪˈnɜ:ʃə]惯性
 *
...
+ (NSArray *)grey_touchPathWithStartPoint:(CGPoint)startPoint
  ...
  if (isnan(duration)) {
    ...
    for (NSUInteger i = 0; i < totalPoints; i++) {
      CGPoint touchPoint = CGPointMake(startPoint.x + (deltaX * i),
...
//    添加惯性点
  ...
  if (cancelInertia) {
    // To cancel inertia, slow down as approaching the end point.
    // This is done by inserting a series of points between the 2nd last and the last point.
  ...
  if (isnan(kCachedScreenEdgePanDetectionLength)) {
    ...
    if ([popGestureRecognizer isKindOfClass:[UIScreenEdgePanGestureRecognizer class]]) {
      ...
      kCachedScreenEdgePanDetectionLength =
                edgeRegionSizeIMP(popGestureRecognizer,
