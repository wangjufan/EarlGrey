...
// Timeout for waiting for rotation to start
// and then again waiting for app to idle after it completes.
...
+ (BOOL)rotateDeviceToOrientation:(UIDeviceOrientation)deviceOrientation
  ...
  BOOL success = [[GREYUIThreadExecutor sharedInstance]
                  executeSyncWithTimeout:kRotationTimeout //from the point of start to the point of stop
                            block:^{
...
- (void)grey_beginTouchesAtPoints:(NSArray *)points
  ...
  GREYFatalAssertWithMessage(!_touchInjector, @"Cannot call this method more than once until endTouch is called.");
