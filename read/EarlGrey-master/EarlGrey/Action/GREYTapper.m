...
+ (BOOL)tapOnElement:(id)element
  ...
  GREYThrowOnFailedCondition(numberOfTaps > 0);
    ...
    // how we navigate from a view to the window backed it ?
  UIView *viewToTap =
 ...
 *          @c element.
 *  location in element --> location in window
 */
+ (CGPoint)grey_tapPointForElement:(id)element
                relativeLocation:(CGPoint)location {
  ...
  CGPoint tapPoint;
  if (viewToTap != element) {//parent holder of location
