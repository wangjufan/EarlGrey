...
/**     拖拽 any direction , maybe complex of two
 *  Generates a touch path in the @c window
 *   from the given @c startPoint and the given @c endPoint.
...
/** pure direction touches path  , and max length
 *  Generates a touch path in the @c window from the start point,
 *  in the given direction to the max possible extent.
 *
 ...
 *  @return NSArray of CGPoints that denote the points in the touch path.
 */
+ (NSArray *)touchPathForGestureWithStartPoint:
 ...
 *
 *  @return Array of CGPoints that denote the points in the touch path.
 *          The touch path's length will be at least the minimum scroll detection length,
 *          when that is not possible (due to @c view position and/or size) @c nil is returned.
 */
