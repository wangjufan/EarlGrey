 ...
 *  and returns all matched UI elements.
 *  The given search action is performed until an element is found.
 *
 *  @param timeout The amount of time during which search actions must be performed to find an
 *                 element.
 *  @param error   The error populated on failure.
 *                 If an element was found and returned when using the search actions
 *                 then any action or timeout errors that happened in the previous search are ignored.
 *                 However, if an element is not found, the error will be propagated.
 *
