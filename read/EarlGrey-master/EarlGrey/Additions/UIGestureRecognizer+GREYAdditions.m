...
+ (void)load {
  @autoreleasepool {
    ...
    GREYFatalAssertWithMessage(swizzled,@"Failed to swizzle UIGestureRecognizer _resetGestureRecognizer");
...
- (void)greyswizzled_resetGestureRecognizer {
  ...
  INVOKE_ORIGINAL_IMP(void,
                      @selector(greyswizzled_resetGestureRecognizer));
}
- (void)greyswizzled_setState:(UIGestureRecognizerState)state {
  ...
  if (state == UIGestureRecognizerStateFailed) {
    GREYAppStateTrackerObject *object = objc_getAssociatedObject(self, @selector(greyswizzled_setState:));
    UNTRACK_STATE_FOR_OBJECT(kGREYPendingGestureRecognition, object);
    objc_setAssociatedObject(self,
                    @selector(greyswizzled_setState:),
                    nil,
