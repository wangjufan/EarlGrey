...
/**
 *  Stack of XCTestCase objects being being executed.
 *  This enables the tracking of different nested tests that have been invoked. 使，跟踪触发的不同的嵌套测试，成为可能
 *  If empty, then the run is outside the context of a running test.
 *  如果为空，则该次运行超出运行的测试的上下文
 */
...
+ (void)load {
  @autoreleasepool {
    ...
    // As soon as XCTest is loaded,
    // we setup the EarlGrey crash handlers so that any issue is tracked at the earliest.
    // Also, we turn on accessibility for the simulator since it needs to be enabled before main is called.
...
////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////
+ (XCTestCase *)grey_currentTestCase {//public
...
- (void)grey_markAsFailedAtLine:(NSUInteger)line
    ...
    // If the test fails outside of the main thread in a nested runloop ,
    // it will not be interrupted until it's back in the outer most runloop.
    // Raise an exception to interrupt the test immediately
