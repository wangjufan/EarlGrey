...
/**
 *  Posted immediately prior to XCTestCase::setUp.
 *  The @c userInfo dictionary contains the executing XCTestCase.
 */
UIKIT_EXTERN NSString *const kGREYXCTestCaseInstanceWillSetUp;
/**
 *  Posted immediately after XCTestCase::setUp is called.
 *  The @c userInfo dictionary contains the executing XCTestCase.
 */
UIKIT_EXTERN NSString *const kGREYXCTestCaseInstanceDidSetUp;

/**
 *  Posted immediately prior to XCTestCase::tearDown.
 *  The @c userInfo dictionary contains the executing XCTestCase.
 */
UIKIT_EXTERN NSString *const kGREYXCTestCaseInstanceWillTearDown;
/**
 *  Posted immediately after XCTestCase::tearDown is called.
 *   The @c userInfo dictionary contains the executing XCTestCase.
 */
UIKIT_EXTERN NSString *const kGREYXCTestCaseInstanceDidTearDown;

/**
 *  Posted immediately after XCTestCase::invokeTest is executed successfully, denoting that the test has passed.
 *  The @c userInfo dictionary contains the executing XCTestCase.
 */
UIKIT_EXTERN NSString *const kGREYXCTestCaseInstanceDidPass;
/**
 *  Posted immediately after XCTestCase::invokeTest raises an Exception, denoting that the test has failed.
 *  The @c userInfo dictionary contains the executing XCTestCase.
 */
UIKIT_EXTERN NSString *const kGREYXCTestCaseInstanceDidFail;

/**
 *  Posted immediately after a XCTestCase finishes, successfully or not.
 *  The @c userInfo dictionary contains the executing XCTestCase.
 */
...
/**
 *  Extends XCTestCase with capabilities to
 *    ==== return current testcase
 *    ==== and allows observing various states of test execution.
 *    ==== Also allows clearing various states
 *  that can leak across from one testcase to another.
 */
@interface XCTestCase (GREYAdditions)
/**
 *  @return The current XCTestCase being executed
 *           or @c nil if called outside the context of a test method.
 */
...
/**
 *  Interrupts the current test case execution immediately and
 *  triggers XCTest's error handling mechanism to
 *  invoke the appropriate methods to tear down the test.
 *
