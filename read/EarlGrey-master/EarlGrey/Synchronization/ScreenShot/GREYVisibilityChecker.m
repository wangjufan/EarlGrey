...
/**
 *  The minimum number of points that must be visible
 *      along with the activation point
 *      to consider an element visible.
 *  It is non-static to make it visible in tests.
 */
const NSUInteger kMinimumPointsVisibleForInteraction = 10;
/**
 *  Last known original image      used by the visibility checker
 *  is saved in this global for debugging purposes.
 */
static UIImage *gLastActualBeforeImage;
/**
 *  Last known actual color shifted image   used by visibility checker
 *  is saved in this global for debugging purposes.
 */
static UIImage *gLastActualAfterImage;
/**
 *  Last known color shifted image    created by the visibility checker
 *  is saved in this global for debugging purposes.
 */
...
BOOL GREYVisibilityDiffBufferIsVisible(GREYVisibilityDiffBuffer buffer, size_t x, size_t y) {
  ...
  return buffer.data[y * buffer.width + x];//line first
}
...
+ (CGPoint)visibleInteractionPointForElement:(id)element {
  ...
  if (viewIntersectsScreen) {
    ...
    GREYVisiblePixelData visiblePixels =
            [self grey_countPixelsInImage:afterImage
                    thatAreShiftedPixelsOfImage:beforeImage
...
+ (double)grey_percentViewVisibleOnScreen:(UIView *)view
  ...
  if (viewIntersectsScreen) {
    ...
    GREYVisiblePixelData visiblePixelData =
          [self grey_countPixelsInImage:afterImage
                    thatAreShiftedPixelsOfImage:beforeImage
...
/**
 *  Calculates the number of pixel in @c afterImage that have different pixel intensity in @c beforeImage.
...
+ (GREYVisiblePixelData)grey_countPixelsInImage:(CGImageRef)afterImage
                ...
                andStoreComparisonResultInBuffer:
                            (GREYVisibilityDiffBuffer *)outDiffBufferOrNULL {
