 ...
 */
+ (BOOL)isNotVisible:(_Nullable id)element; // invisible for every-part of the elements
/**
 *  @return @c YES if at least 10 (@c kMinimumPointsVisibleForInteraction) points are visible @b
 *           and the activation point of the given element is also visible, @c NO otherwise.
 */
...
/**  screen coordinates
 *  @return The smallest rectangle enclosing the entire visible area of @c element in screen coordinates.
 *          If no part of the element is visible, CGRectZero will be returned.
 *          The returned rect is always in points.
 */
