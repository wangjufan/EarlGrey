...
/////////////////////////////////////////////////////////////////////////////////////
/**
 *  Blocking call   that drains the main runloop enough times   to make each source gets a fair chance of service.
 *  No guarantee is made on whether the app is in kGREYIdle state after this method returns.
 */
- (void)drainOnce;//No guarantee kGREYIdle state when returns
/**
 *  Blocking call   that drains the UI thread for the specified number of @c seconds.
 *  This method can block for longer than the specified time if any of the signalled sources take longer than that to execute.
 *  @param seconds Amount of time that the UI thread should be drained for, in seconds.
 */
- (void)drainForTime:(CFTimeInterval)seconds;//  kGREYIdle state when returns
/**
 *  Blocking call  that drains the UI thread   until both the UI and registered GREYIdlingResources
 *  are in idle.
 *  @remark Be very careful while calling this as you could end up in state-----
 *          where the caller expects the callee to mark the thread as idle
 *          and callee inadvertantly calls GREYUIThreadExecutor::drainUntilIdle:,
 *           in which case it will go into an infinite loop and the test will have to be force-killed by the test-runner.
 */
- (void)drainUntilIdle;
/**
 ...
 */
- (BOOL)drainUntilIdleWithTimeout:(CFTimeInterval)seconds;//condition idle both ui and resources
/////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////
/////////////////////////////////////////////////////////////////////////////////////
/**
