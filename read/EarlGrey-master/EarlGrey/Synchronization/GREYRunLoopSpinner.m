...
- (void)grey_drainRunLoopInActiveModeForDrains:(NSUInteger)exitDrainCount {
  ...
  void (^wakeUpBlock)() = ^{// before drained count , never sleep
  ...
  while (drainCount < exitDrainCount) {
    @autoreleasepool {
      ...
      if (result == kCFRunLoopRunFinished) {
        // Running a run loop mode -- will finish if that mode has no sources or timers.
        // In that case, the observer callbacks will not get called, so we need to increment the drain count here.
        drainCount++;
      }
...
/**
 *  Spins the run loop in the active mode ,
 *  until the stop condition has been met, we have timed out,
 *        the run loop finishes, or the run loop is stopped by someone else.
 *  Checks the stop condition at least once per run loop drain.
 *
 ...
 *  @return @c YES if the condition block was evaluated to YES while draining or after the active run loop finished; @c NO otherwise.
 */
- (BOOL)grey_drainRunLoopInActiveModeAndCheckCondition:
                                    (BOOL (^)())stopConditionBlock
 ...
 *  @param beforeSourcesBlock -- Block that will be invoked when the added observer recieves before-sources callbacks and is not nested.
 *  @param beforeWaitingBlock -- Block that will be invoked when the added observer recieves before-waiting callbacks and is not nested.
