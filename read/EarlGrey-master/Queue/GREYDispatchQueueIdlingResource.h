 ...
 *  @c dispatch_after blocks and @c dispatch_after_f tasks sent to @c queue are tracked if they are delayed no more than the delay amount set for the  @c kGREYConfigKeyTrackableDispatchAfterDuration configuration.
 *  A weak reference is held to @c queue. If @c queue is deallocated, then the idling resource will deregister itself from the UI thread executor.
